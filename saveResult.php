<?php
    ...
    if($count > 0){
        $queryRecords="UPDATE `records` SET `game_record`='$winner_game_record',`match_rocord`='$winner_Match_Record' , `place`='$winnerPlaceDB'  WHERE `group_id`='$group_id' AND `player_id`='$winner_id' AND `group_id`='$group_id'";
    ...
    $count=  mysqli_num_rows($result);
    if($count > 0){
        $queryRecords="UPDATE `records` SET `game_record`='$looser_game_record',`match_rocord`='$looser_Match_Record' , `place`='$looserPlaceDB'  WHERE `group_id`='$group_id' AND `player_id`='$looser_id' AND `group_id`='$group_id'";
        $q = mysqli_query($con, $queryRecords);
        if($q){ $flag=TRUE; } else { $flag=FALSE; }
    }
    else{
