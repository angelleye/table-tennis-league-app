...
$query = "SELECT * FROM `event` where event_id='" . $_GET["id"] . "'";
...
while ($records = mysqli_fetch_array($result)) {
    ...
    $event_db_date=$records['event_date'];
}
        ...
        <div class="container">
            ...
            <form class="form-inline pull-right">
                <div class="form-group">
                    <label for="email">Event Date : <?php echo $event_db_date; ?></label>
                </div>
            </form>
                                ...
                                if ($j == -1) {
                                    ...
                                    for ($k = 0; $k < count($finalArray[$i]); $k++) {
                                        if ($k == $j) {
                                            ...
                                            $getrecordsQuery = "SELECT `winner_game_count`,`looser_game_count`,winner_id,looser_id FROM `result_tt` WHERE `event_id`='" . $_GET["id"] . "' AND `group_id`='G" . ($i + 1) . "' AND (`winner_id`='{$u}' OR `looser_id`='{$u}') AND (`winner_id`='{$finalArray[$i][$k]}' OR `looser_id`='{$finalArray[$i][$k]}')";
                                    ...
                                    $query_tt = "SELECT `records`.`game_record` , `records`.`match_rocord`,`records`.`place` 
                                                ...
                                                where users.user_id='$u' AND records.group_id='G" . ($i + 1) . "' AND records.event_id='" . $_GET["id"] . "'";
                ...
                <div class="modal-content">
                    <form role="form" id="addUserForm" method="post" action="addFlyUser.php">
                        ...
                        <div class="modal-body">
                            ...
                            <input type="hidden" name="eventId" value="<?php echo $_GET["id"]; ?>" id="hiddenEventId" />
                            <div class="form-group">
